...
OBSERVATIONS_URL = "#{BASE_URL}/s/scarcroft-green-nursery/observation"
...
def get_doc(observation_id)
  ...
  url = "#{OBSERVATIONS_URL}/#{observation_id}"
...
def get_next_observation_id(doc)
  ...
  next_link.attribute('href')
           .value
           .match(%r{#{Regexp.quote(OBSERVATIONS_URL)}/(\d*)})
           .captures
           .first
end

t = Tapestry.new
observation_id = t.first_observation_id
